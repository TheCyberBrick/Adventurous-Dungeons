...
public class CatmullRomSpline implements ISpline {
	...
	public Vec3d interpolate(float s) {
		...
		double A1x = P0.xCoord * A1Mul1 + P1.xCoord * A1Mul2;
		double A1y = P0.yCoord * A1Mul1 + P1.yCoord * A1Mul2;
		double A1z = P0.zCoord * A1Mul1 + P1.zCoord * A1Mul2;

		double A2Mul1 = (t2 - t) / (t2 - t1);
		double A2Mul2 = (t - t1) / (t2 - t1);

		double A2x = P1.xCoord * A2Mul1 + P2.xCoord * A2Mul2;
		double A2y = P1.yCoord * A2Mul1 + P2.yCoord * A2Mul2;
		double A2z = P1.zCoord * A2Mul1 + P2.zCoord * A2Mul2;

		double A3Mul1 = (t3 - t) / (t3 - t2);
		double A3Mul2 = (t - t2) / (t3 - t2);

		double A3x = P2.xCoord * A3Mul1 + P3.xCoord * A3Mul2;
		double A3y = P2.yCoord * A3Mul1 + P3.yCoord * A3Mul2;
		double A3z = P2.zCoord * A3Mul1 + P3.zCoord * A3Mul2;
	...
	public Vec3d derivative(float s) {
		...
		double dA1x = (P1.xCoord - P0.xCoord) * dA1Mul;
		double dA1y = (P1.yCoord - P0.yCoord) * dA1Mul;
		double dA1z = (P1.zCoord - P0.zCoord) * dA1Mul;

		double dA2x = (P2.xCoord - P1.xCoord) * dA2Mul;
		double dA2y = (P2.yCoord - P1.yCoord) * dA2Mul;
		double dA2z = (P2.zCoord - P1.zCoord) * dA2Mul;

		double dA3x = (P3.xCoord - P2.xCoord) * dA3Mul;
		double dA3y = (P3.yCoord - P2.yCoord) * dA3Mul;
		double dA3z = (P3.zCoord - P2.zCoord) * dA3Mul;
	...
	public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
		NBTTagList ptsNbt = new NBTTagList();
		for(Vec3d pt : this.pts) {
			ptsNbt.appendTag(MoreNBTUtils.writeVec(pt));
		}
		nbt.setTag("pts", ptsNbt);
		return nbt;
	}

	public static CatmullRomSpline readFromNBT(NBTTagCompound nbt) {
		if(nbt.hasKey("pts", Constants.NBT.TAG_LIST)) {
			NBTTagList ptsNbt = nbt.getTagList("pts", Constants.NBT.TAG_COMPOUND);
			Vec3d[] pts = new Vec3d[ptsNbt.tagCount()];
			for(int i = 0; i < ptsNbt.tagCount(); i++) {
				pts[i] = MoreNBTUtils.readVec(ptsNbt.getCompoundTagAt(i));
			}
			return new CatmullRomSpline(pts);
		}
		return null;
	}
}
