...
public class ReparameterizedSpline implements ISpline {
	...
	public void init(int subdivs, int segmentSubdivs, double epsilon, int maxNewtonIter) {
	 ...
	 */
	public void init(int subdivs, int segmentSubdivs) {
	...
	private float getInterpolatedCurveParameter(double s) {
		...
		if(s <= 0.0D) {
			...
			parameter = (float)this.reparameterizedInterpol.interpolate((float)(s / this.length)).xCoord;
		}
		return parameter;
	}
