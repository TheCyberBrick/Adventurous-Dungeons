...
/**
 * This component divides a vector by a value
 */
public class DivVecSC extends DungeonScriptComponent {
	private InputPort<Vec3d> in_vec;
	private InputPort<Double> in_div;
	private OutputPort<Vec3d> out;

	public DivVecSC(Script script) {
		super(script);
	}

	public DivVecSC(Script script, String name) {
		super(script, name);
	}
	...
	protected void createPorts() {
		...
		this.in_div = this.in("div", Double.class, true);
		this.out = this.out("out", Vec3d.class);
	}

	@Override
	protected void run() throws ScriptException {
		this.put(this.out, this.get(this.in_vec).scale(1.0D / this.get(this.in_div)));
	}
}
