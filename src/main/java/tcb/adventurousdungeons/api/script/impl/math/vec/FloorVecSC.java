...
/**
 * This component rounds down the values of a Vec3d
 */
public class FloorVecSC extends DungeonScriptComponent {
	private InputPort<Vec3d> in;
	private OutputPort<Vec3d> out;

	public FloorVecSC(Script script) {
		super(script);
	}

	public FloorVecSC(Script script, String name) {
		super(script, name);
	}
	...
	protected void run() throws ScriptException {
		...
		this.put(this.out, new Vec3d(MathHelper.floor(in.xCoord), MathHelper.floor(in.yCoord), MathHelper.floor(in.zCoord)));
	}
}
