...
public class Script {
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		for(int i = 0; i < componentsNbt.tagCount(); i++) {
			...
			try {
				IScriptComponent component = ScriptComponentRegistry.INSTANCE.createComponent(type, this);
				if(component == null) {
		...
		for(int i = 0; i < connectionsNbt.tagCount(); i++) {
			...
			IScriptComponent component = this.getComponent(componentID);
			if(component != null) {
				for(OutputPort<?> port : component.getOutputs()) {
					if(connectionNbt.hasKey("output." + port.getID(), Constants.NBT.TAG_COMPOUND)) {
						...
						IScriptComponent inputComponent = this.getComponent(inputComponentID);
